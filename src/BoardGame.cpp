...
void BoardGame::assignPlayerShips(int player, std::vector<int>& shipPoints, char sym)
{
...
void BoardGame::assignComputerShips()
{
	...
	for (int z = 0; z < SHIP_SIZE; z++)
	{
		do
		{
			...
			{
				passValue = (std::to_string((endPoints[i] * 10) + endPoints[i + 1]));
...
bool BoardGame::checkShipStatus(char checkValue, int player)
{
	...
	std::string checkLine = listOfBoards[playerNumber].toString('b');
...
std::vector<std::string> BoardGame::getBoardSetup(int player)
{
	std::vector<std::string> tempSetup = std::vector<std::string>();
